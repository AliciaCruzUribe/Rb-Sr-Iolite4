...
def exportResults(results_dict):
    """Export regression results to Excel or text file."""
    if not results_dict:
...
def exportToText(results_dict, file_path):
    ...
    try:
        with open(file_path, 'w', encoding='utf-8') as f:
            ...
            if results_dict.get('prob') is not None:
                f.write(f"Probability: {results_dict.get('prob', 0):.6f}\n")
            f.write(f"Degrees of Freedom: {results_dict.get('dof', 0)}\n")
            f.write(f"Number of Points: {results_dict.get('n_points', 0)}\n")
...
def updateSecondaryRMAge(rm_name, age_line_edit, age_2se_line_edit=None, intercept_line_edit=None, intercept_uncert_line_edit=None):
    """Update the age and intercept fields when a different secondary RM is selected."""
    try:
        rm_data = data.referenceMaterialData(rm_name)
        if "Age" in rm_data:
            age_line_edit.setText(str(float(rm_data["Age"].value())))
            drs.setSetting("SecondaryRMAge", float(rm_data["Age"].value()))
            if age_2se_line_edit and "Age" in rm_data:
                age_2se_line_edit.setText(str(float(rm_data["Age"].uncertainty())))
                drs.setSetting("SecondaryRMAge2SE", float(rm_data["Age"].uncertainty()))
        if intercept_line_edit and "87Sr/86Sr initial" in rm_data:
            intercept_line_edit.setText(str(float(rm_data["87Sr/86Sr initial"].value())))
            drs.setSetting("FixedIntercept", float(rm_data["87Sr/86Sr initial"].value()))
        if intercept_uncert_line_edit and "87Sr/86Sr initial" in rm_data:
            intercept_uncert_line_edit.setText(str(float(rm_data["87Sr/86Sr initial"].uncertainty())))
            drs.setSetting("FixedInterceptUncert", float(rm_data["87Sr/86Sr initial"].uncertainty()))
    except Exception as e:
        print(f"Could not load RM data for {rm_name}: {e}")
...
def runDRS():
    ...
    # Determine which post-shift masses to look for based on N2O setting
    use_n2o = settings.get("UseN2OMassShift", False)
    if use_n2o:
        # N2O/SrO: mass shift +16 -> 86Sr at 102, 87Sr at 103, 88Sr at 104
        sr86_target = '102'
        sr87_target = '103'
        sr88_target = '104'
        print("Using N2O mass shift (+16): looking for Sr masses 102, 103, 104")
    else:
        # SF6/SrF: mass shift +19 -> 86Sr at 105, 87Sr at 106, 88Sr at 107
        sr86_target = '105'
        sr87_target = '106'
        sr88_target = '107'
        print("Using SF6 mass shift (+19): looking for Sr masses 105, 106, 107")

    # Helper: find a CPS channel by post-shift mass, trying property lookup first,
    # then falling back to name-based matching (e.g. "Sr86_102_CPS" contains "_102_")
    def find_cps_channel(target_mass, element_hint):
        """Find an intermediate CPS channel for a given post-shift mass.
        element_hint is used for name matching (e.g. 'Rb85', 'Sr86')."""

        # Method 1: Try property-based lookup (works when file has mass properties)
        matches = data.timeSeriesList(data.Intermediate, {'Post-shift mass': target_mass})
        if matches:
            print(f"  Found {element_hint} via property lookup: {matches[0].name}")
            return matches[0].data()

        # Method 2: Fall back to name-based matching on _CPS channels
        # For shifted channels, look for names like "Sr86_102_CPS" (element_mass_CPS)
        # For unshifted channels, look for names like "Rb85_CPS"
        all_intermediate = data.timeSeriesList(data.Intermediate)

        # First try: look for "ElementXX_TargetMass_CPS" pattern (e.g. "Sr86_102_CPS")
        for ch in all_intermediate:
            if ch.name.endswith('_CPS') and f'_{target_mass}_' in ch.name:
                print(f"  Found {element_hint} via name pattern '_{target_mass}_': {ch.name}")
                return ch.data()

        # Second try: for unshifted masses like Rb85, look for "Rb85_CPS" (no second number)
        for ch in all_intermediate:
            if ch.name == f'{element_hint}_CPS':
                print(f"  Found {element_hint} via exact name: {ch.name}")
                return ch.data()

        # Nothing found
        available = [ch.name for ch in all_intermediate if ch.name.endswith('_CPS')]
        IoLog.error(f"Could not find channel for {element_hint} at post-shift mass {target_mass}. "
                    f"Available CPS channels: {available}")
        return None
    ...
    int_channel_names = ['Rb87_CPS', 'Rb85_Sr86s_Raw', 'Sr87s_Sr86s_Raw',
                         'Rb87_Sr86s_Raw', 'Sr87s_Rb87_Raw', 'Sr88s_Sr86s_Raw', 'Sr87m_Sr86m_Raw', 'Rb87_Sr86m_Raw']
    ...
    Rb87_Sr86_uncert = np.array(Rb87_Sr86_uncerts)
    Sr87_Sr86_uncert = np.array(Sr87_Sr86_uncerts)

    # Convert 2SE to 1σ for York regression (York expects 1-sigma uncertainties)
    # 2SE = 2 * SE, so 1σ = 2SE / 2 = SE
    Rb87_Sr86_uncert_1sigma = Rb87_Sr86_uncert / 2.0
    Sr87_Sr86_uncert_1sigma = Sr87_Sr86_uncert / 2.0
    ...
    fixed_intercept_uncert_1se = fixed_intercept_uncert_2se / 2.0  # Convert to 1SE
    ...
    # Calculate error correlation if enabled
    if use_corr_errors and len(Rb87_Sr86_sec) >= 2:
        rho = np.corrcoef(Rb87_Sr86_sec, Sr87_Sr86_sec)[0, 1]
        rho_array = np.full(len(Rb87_Sr86_sec), rho)
        print(f"Using correlated errors: rho = {rho:.4f}")
    else:
        rho_array = np.zeros(len(Rb87_Sr86_sec))
        print("Using uncorrelated errors (r=0)")

    # Convert 2SE to 1SE for Models 2 and 3 (which expect 1SE)
    Rb87_Sr86_uncert_1se = Rb87_Sr86_uncert / 2.0
    Sr87_Sr86_uncert_1se = Sr87_Sr86_uncert / 2.0
    ...
    try:
        if regression_model == 0:
            ...
            slope, intercept, slope_uncert_2se, intercept_uncert_2se, mswd, prob, n_reg, degrees_of_freedom, cov_ab = york_regression(
                ...
                Rb87_Sr86_uncert, Sr87_Sr86_uncert,  # 2SE
                rho=rho_array,
                fix_intercept=use_fixed_intercept,
                fixed_int=fixed_intercept if use_fixed_intercept else None,
                fixed_int_err=fixed_intercept_uncert_1se if use_fixed_intercept else None
            )
            # Convert to 1SE for age calculation
            ...
            if use_fixed_intercept:
                slope, slope_uncert, n_reg, degrees_of_freedom = tls_regression_fixed_intercept(
                    Rb87_Sr86_sec, Sr87_Sr86_sec, fixed_intercept, fixed_intercept_uncert_1se
                )
                intercept = fixed_intercept
                intercept_uncert = 0.0
            else:
                slope, intercept, slope_uncert, intercept_uncert, n_reg, degrees_of_freedom = tls_regression_free_intercept(
                    Rb87_Sr86_sec, Sr87_Sr86_sec
                )
            mswd = None
            prob = None
            ...
            if use_fixed_intercept:
                slope, slope_uncert, mswd, prob, dispersion, degrees_of_freedom = york_overdispersion_fixed_intercept(
                    ...
                    fixed_intercept, fixed_intercept_uncert_1se, r=rho_array
                )
                intercept = fixed_intercept
                intercept_uncert = 0.0
                dispersion_err = None
                ...
                slope, intercept, slope_uncert, intercept_uncert, mswd, prob, dispersion, dispersion_err, degrees_of_freedom = york_overdispersion_free_intercept(
                    Rb87_Sr86_sec, Sr87_Sr86_sec,
                    Rb87_Sr86_uncert_1se, Sr87_Sr86_uncert_1se, r=rho_array
                )
    ...
    if regression_model == 0:
        ...
        # If MSWD > 1 and p < 0.05, multiply by sqrt(MSWD)
        alpha = 0.05
        if prob is not None and prob < alpha and mswd is not None and mswd > 1:
            t_value = stats.t.ppf(1 - alpha/2, degrees_of_freedom)
            slope_uncert_scaled = slope_uncert * np.sqrt(mswd)
            print(f"  MSWD scaling applied: sqrt({mswd:.4f}) = {np.sqrt(mswd):.4f}")
        else:
            t_value = stats.norm.ppf(1 - alpha/2)
            slope_uncert_scaled = slope_uncert
        age_uncert_ma = t_value * slope_uncert_scaled / (slope + 1) / lambda_rb87 / 1e6

    elif regression_model == 1:
        # Model 2: TLS uses normal distribution
        t_value = stats.norm.ppf(0.975)
        age_uncert_ma = t_value * slope_uncert / (slope + 1) / lambda_rb87 / 1e6

    elif regression_model == 2:
        # Model 3: IsoplotR simply multiplies 1SE by 2 for 95% CI
        age_uncert_ma = 2.0 * slope_uncert / (slope + 1) / lambda_rb87 / 1e6
    ...
    # Handle None values for Model 2 (TLS) which doesn't calculate MSWD/prob
    ...
    secondary_rm_age_2se = settings.get("SecondaryRMAge2SE", 0.0)
...
def settingsWidget():
    ...
    drs.setSetting("ReferenceMaterial", "G_Phl-120-G")
    ...
    drs.setSetting("FixedIntercept", 0.7043)
    drs.setSetting("FixedInterceptUncert", 0.0002)
    drs.setSetting("UseFixedIntercept", True)
    ...
    # N2O mass shift checkbox
    n2oCheckBox = QtGui.QCheckBox(leftWidget)
    n2oCheckBox.setChecked(settings.get("UseN2OMassShift", False))
    ...
    intercept_value = float(settings.get("FixedIntercept", 0.7043))
    ...
    intercept_uncert_value = float(settings.get("FixedInterceptUncert", 0.0002))
    ...
    # Connect the combo box to update the settings and fields
